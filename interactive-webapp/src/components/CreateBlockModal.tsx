    ...
    <div className="absolute top-0 left-0 w-screen h-screen bg-black bg-opacity-50 justify-center items-center z-40 flex flex-col">
      ...
      <div className="flex flex-col w-96">
        ...
        <input
          type="text"
          id="blockInputs"
          value={blockInputs.join(",")}
          onChange={(e) => setBlockInputs(e.target.value.split(","))}
          className="text-primary-background"
        ...
        <input
          type="text"
          id="blockOutputs"
          value={blockOutputs.join(",")}
          onChange={(e) => setBlockOutputs(e.target.value.split(","))}
          className="text-primary-background"
        ...
        <textarea
          id="blockFn"
          value={blockFn}
          onChange={(e) => setBlockFn(e.target.value)}
          className="text-primary-background"
          placeholder="([a, b]: number[]) => a + b"
        />
      </div>
      <div className="flex justify-between w-96 mt-4">
        <button
          className="px-3 py-2 bg-danger rounded text-primary-background"
          onClick={cancelCreateBlock}
        >
          Cancel
        </button>
        <button
          className="px-3 py-2 bg-success rounded text-primary-background"
